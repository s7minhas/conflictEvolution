...
################
rfOutSamp = function(rfForm, cores=4, folds=30, seed=6886){
	################
...
tmp = rocPlot(rocData, linetypes=ggLty, colorManual=ggCols) + 
	guides(linetype = FALSE, color = FALSE) ; yLo = -.04 ; yHi = .24
...
ggsave(tmp, 
	file=paste0(pathResults, 'figureA5a.pdf'), 
	width=5, height=5)
...
ggsave(tmp, 
	file=paste0(pathResults, 'figureA5b.pdf'),
	width=5, height=5)	
################################################