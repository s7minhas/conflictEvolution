...
To improve on these issues, we subset the data to include the raw text available from the larger ICEWS  database. Then, using a subset of cases from 2006-2012, we reviewed each individual case. This task includes two main goals: to label events as they relate to specific drug cartel actors in the area and to address aggregation problems in the data. In addition, we coded for duplicate cases.  We identified that this data has fewer duplication issues than previously found in other ICEWS data (such as protests) but has a number of complex aggregation and parsing problems. 
...
\noindent These matrices reveal a variety of interesting dynamics in the data, as shown in the network graphs of Figure~\ref{fig:all}. For simplicity, the actors are simply coded as DTO or Government actors. Government actors include different branches of the armed forces and government.

In 2007 to 2008, shown in Figure~\ref{fig:all}, the number of conflictual events increase. Both federal agencies and municipal forces are more active in the conflict.  We also observe a jump in the number of cartels involved in the network, as new actors such as the inclusion of the Juarez Cartel and the increase in events associated with La Familia Micho\'acana.

In the 2008 period, the networks reflect a change in the government policy at that time. By 2006 Calder\'{o}n was elected president and implemented a militarized strategy, sending federal police and  troops into the most contested regions. 

\begin{figure}[h!]
\includegraphics[scale=.45]{graphsAll}
\caption{
The evolution of the Mexican criminal conflict, 2007-2008. Orange nodes are government actors, teal corresponds to criminal organizations. The links (grey lines) are weighted by the number of conflictual events for that year. 
}
\label{fig:all}
\end{figure}

Overall, density increases across these networks overtime.  
