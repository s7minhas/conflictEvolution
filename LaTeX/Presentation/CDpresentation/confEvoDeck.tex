...
\frame{
 ...
 \item Disaggregating actors (Weinstein 2007, Shellman et al, 2010)
\end{itemize}
}
...
\frame{
  \frametitle{Network Effects \& Social Structure}
 Missing information in previous work:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{
  \frametitle{Network Phenomena: Second-order effect (Reciprocity)}
...
\section{Model: The Latent Factor Model}
...
\begin{aligned}
      ...
      \color{amaranth}{\{ (a_{1}, b_{1}), \ldots, (a_{n}, b_{n}) \}} &  \sim N(0, \color{amaranth}{\Sigma_{ab})} \\ 
      \{ (\epsilon_{ij}, \epsilon_{ji}) : \; i \neq j\} &\sim N(0,\Sigma_{\epsilon}), \text{ where } \\
...
\frame{
\frametitle{Recap: Expectations for the Nigerian Case}
How do network dynamics influence the likelihood of conflict? 
\begin{itemize}
 	\item Sender/reciever effects
	\item Reciprocity 
	\item Homophily \& stochastic equivalence 
	\item Key Actor entry: aggressive new actors signal government weakness
\end{itemize}
}
...
\frametitle{Key Take-Aways }
  ...
  \vspace{.08in}
   ...
   \textbf{\textcolor{seafoam}{\textsc{confirmed}}}: Key players alter violence in the conflict system, even in warring dyads that the key player is not directly involved.
  \vspace{.08in}
 ...
 \frame{
   \frametitle{Future Work: Conflict Processes Revealed}
