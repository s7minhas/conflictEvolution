...
vioContraCiv = summaryBy(x+FATALITIES~YEAR+a1,
	data=nData[nData$EVENT_TYPE=='Violence against civilians',], FUN=sum)
...
xNodeL = lapply(1:length(yList), function(t){
	...
	# add in combo
	nodeMat = cbind(nodeMat, riotsProtestsAgainst=(nodeMat[,'riotsAgainst'] + nodeMat[,'protestsAgainst']))

	# add in concatenations with vig militia
	if('Vigilante Militia (Nigeria)' %in% rownames(y)){
		vigActions = y['Vigilante Militia (Nigeria)',]
	} else { vigActions = 0 }
	nodeMat = cbind(nodeMat, 
		riotsAgainst_inclVig=(nodeMat[,'riotsAgainst'] + vigActions),
		protestsAgainst_inclVig=(nodeMat[,'protestsAgainst'] + vigActions),
		riotsProtestsAgainst_inclVig=(nodeMat[,'riotsProtestsAgainst'] + vigActions)
		)
