...
for(id in ids){ nData[,id][which(grepl('Military Forces of Nigeria',nData[,id]))] = 'Military Forces of Nigeria' }
for(id in ids){ nData[,id][which(grepl('Police Forces of Nigeria',nData[,id]))] = 'Police Forces of Nigeria' }

# nscdc paramilitary arm of nigeria
for(id in ids){ nData[,id][nData[,id]=='NSCDC: Nigeria Security and Civil Defence Corps'] = 'Military Forces of Nigeria' }

# create master Oodua Peoples Congress category
for(id in ids){ nData[,id][nData[,id]=="Mutiny of OPC: Oodua Peoples Congress"] = "OPC: Oodua Peoples Congress" }

# create master boko haram category
for(id in ids){ nData[,id][which(grepl('Boko Haram',nData[,id]))] = 'Boko Haram' }
