...
ggVioCiv = ggplot(vioCivCnt, aes(x=year, y=value)) +
	...
	theme(
		...
		axis.text=element_text(family="Source Sans Pro Light",size=8),
		strip.text.x = element_text(size = 10, color='white',
			family="Source Sans Pro Semibold", angle=0, hjust=.05),
...
# 
gArrPos = layout.circle(g) ; rownames(gArrPos) = rownames(yArrSum)
...
lapply(1:length(yList), function(t){
	...
	plotGraph(g, pos, 
		...
		vLabCex=.0001,
		vertex.size=g$vSize, 
		# vertex.label.font=1, vertex.label.family="Helvetica", vLabCol=vLabCol, 
		vertex.label=NA,
