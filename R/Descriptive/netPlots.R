...
noGov = rownames(yArrSum)[-which(rownames(yArrSum) %in% govActors)]
yArrSumNoGov = yArrSum[noGov, noGov]
...
tmp=getGraphfromMat(mat=yArrSumNoGov)
gArrSumNoGov=tmp$g ; gArrPosNoGov=tmp$nodePos ; rm(tmp)

# # 
# fck = c(10, 30, 4, 19, 26, 31, 11, 36, 12, 20, 16, 18) # fulani conflict subset
...
################
# plot by full, pre and post bh periods
fName = paste0(pathGraphics, 'nigeria_2000_2016_grant.pdf') ; pdf(file=fName, width=13,height=9)
...
vCol = ifelse(names(V(gArrSumFulani)) == 'Fulani\nMilitia', 'gray30', 'gray95')
vLabCol = ifelse(names(V(gArrSumFulani)) == 'Fulani\nMilitia', 'white', 'gray30')
...
plotGraph(gArrSumNoGov, gArrPosNoGov, 
	vShape='circle', vLabCex=gArrSumNoGov$labSize+.3, 
	vLabCol=vLabCol, vertex.label.font=1, vertex.size=gArrSumNoGov$vSize, 
	vFrameCol=vCol, vCol=vCol, vertex.label.family="Helvetica")
title('Nigerian Intra-State Conflict\n(2000-2016)\nNo Gov. Actors', family='Helvetica', adj=1, line=-4)	
dev.off() ; system(paste('pdfcrop',fName,fName, sep=' '))
################
