...
designArrays = list(
	...
	rioProSep=list(
		senCovar=subListArray(xNodeL, c('riotsAgainst', 'protestsAgainst','vioCivEvents', 'groupSpread', 'govActor'), 2),
		recCovar=subListArray(xNodeL, c('riotsAgainst', 'protestsAgainst','vioCivEvents', 'groupSpread', 'govActor'), 2),
		dyadCovar=subListArray(xDyadL, c('postBoko', 'elecYear', 'ngbrConfCount'), 3)
		)
	)
...
save(
	...
	file=paste0(pathResults, 'ameResults_interactions.rda')
	)
################