...
if(!file.exists(paste0(pathResults, 'revAppendix/ameCrossValResults_varFoldSize.rda'))){
	results = lapply(foldSizes, function(fSize){
		mod = ameOutSamp(
			yList=yList, 
	...
	save(
		results,
		file=paste0(pathResults, 'revAppendix/ameCrossValResults_varFoldSize.rda')
	) }
################

############################
# org results
load(paste0(pathResults, 'revAppendix/ameCrossValResults_varFoldSize.rda'))
predDfs = lapply(1:length(foldSizes), function(i){
	out = cbind(
		results[[i]]$outPerf, 
		model=paste0('AME (K=',foldSizes[i],')')) })
names(predDfs) = paste0('AME (K=', foldSizes, ')')
...
print.xtable(
	xtable(aucSumm, 
		align='lcc',
		caption='Out-of-sample performance statistics by varying the size of folds in cross-validation.',
	...
	size='normalsize',
	file=paste0(pathResults, 'revAppendix/ame_varFoldSize.tex')
	)
################
################################################